...
import { PenTool, BookOpen, Headphones, ShieldAlert, Sparkles, Activity, Search } from 'lucide-react';
...
const Dashboard = ({ sermons, studies, externalSermons }) => {
    ...
    const filteredResults = searchQuery.trim() === '' ? [] : allItems.filter(item => {
        const query = searchQuery.toLowerCase();
        return (
            (item.title && item.title.toLowerCase().includes(query)) ||
            (item.topic && item.topic.toLowerCase().includes(query)) ||
            (item.scripture && item.scripture.toLowerCase().includes(query)) ||
            (item.scriptures && item.scriptures.toLowerCase().includes(query)) ||
            (item.preacher && item.preacher.toLowerCase().includes(query))
        );
    });
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="relative w-full sm:w-96">
                    <input
                        type="text"
                        placeholder="Search all sermons & studies..."
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none transition font-medium text-gray-800 shadow-sm"
                    />
            ...
            {searchQuery.trim() !== '' ? (
                <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8 relative overflow-hidden animate-in fade-in zoom-in-95 duration-300">
                    <h3 className="text-xl font-extrabold text-gray-900 mb-6 flex items-center gap-2 relative z-10">
                        <Search size={24} className="text-indigo-500" /> Search Results ({filteredResults.length})
                    </h3>

                    {filteredResults.length === 0 ? (
                        <div className="text-center p-8 bg-gray-50 rounded-2xl border border-gray-100 border-dashed">
                            <p className="text-gray-500 font-medium text-lg">No results found for "{searchQuery}".</p>
                            <p className="text-sm text-gray-400 mt-1">Try a different keyword, topic, or scripture.</p>
                        </div>
                    ) : (
                        <div className="space-y-4 relative z-10">
                            {filteredResults.map((item, i) => {
                                ...
                                return (
                                    <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-white hover:bg-gray-50 transition-colors border border-gray-100 shadow-sm hover:shadow-md">
                                        <div className={`p-3 rounded-xl ${item.bg} ${item.color}`}>
                                            <Icon size={20} />
                                        </div>
                                        <div className="flex-1">
                                            <h4 className="font-bold text-gray-900 text-lg">{item.title || item.topic}</h4>
                                            <div className="flex items-center gap-3 mt-1 text-sm">
                                                <span className={`text-[10px] uppercase tracking-wider font-extrabold px-2 py-0.5 rounded-md ${item.bg} ${item.color}`}>
                                                    {item.type}
                                                </span>
                                                <span className="font-semibold text-gray-500 capitalize">{item.date}</span>
                                                {(item.scripture || item.scriptures) && (
                                                    <span className="font-medium text-indigo-500 hidden sm:inline-block">
                                                        • {item.scripture || item.scriptures}
                                                    </span>
                                                )}
                                            </div>
